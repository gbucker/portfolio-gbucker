...
function LinkList({links}: {links: LinkType[]}) {
  return (
    <>
      ...
      {links.map((link) => (
        <div key={link._id} className="bg-white flex items-bottom justify-left gap-10 border-2 border-gray-500 rounded-lg p-1 hover:scale-105 hover:border-purple-500 transition">
        <GradientLink className="flex" href={`/link/${link.slug}`}>
          {link.image && (
            <Image
          ...
          <div className="flex flex-col mt-2 mb-2 grow-0"><h3 className="text-lg font-bold">{link.name}</h3><p className="text-lg">{link.date && `${moment(link.date).format("DD/MM/yyyy")}`}</p></div>
        </GradientLink>
        </div>
      ))}
      </div>
